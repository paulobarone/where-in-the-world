...
export default function Home() {
  ...
  return (
    <>
      {!countries && <LinearProgress className='absolute top-0 left-0 w-screen' />}
      <div>
        <div className="px-5 md:px-24 py-10 flex flex-col gap-3 sm:flex-row sm:justify-between sm:gap-24">
          <form className="relative w-full max-w-lg" onSubmit={handleSearchCountry}>
            ...
            <input className="w-full bg-white rounded-lg outline-none py-3 pl-16 px-6 shadow-lg sm:w-full dark:bg-darkModeElements dark:text-white" placeholder="Search for a country" id="search" value={searchText} onChange={(e) => setSearchText(e.target.value)} autoComplete="off" />
          </form>
